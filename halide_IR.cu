...
/** GPU3 INITIALIZATIONS */
    ...
    for(int c1=0; c1<13; c1++){
        for(int c3  =max(c1-4, 0); c3 < min(c1, 8) - max(c1-4, 0) +1; c3++){
            ...
            // Memcpy GPU0 to GPU1
            #define offset2 ((c3 + 1 - (c1 - c3)) * (SEQ_LENGTH + 1) * BATCH_SIZE * FEATURE_SIZE + ((c1 - c3) * GEMM_BATCH + 1) * BATCH_SIZE * FEATURE_SIZE)
            #define offset3 ((c3 + 1 - (c1 - c3)) * (SEQ_LENGTH/2 + 1) * BATCH_SIZE * FEATURE_SIZE + ((c1 - c3) * GEMM_BATCH + 1) * BATCH_SIZE * FEATURE_SIZE)
